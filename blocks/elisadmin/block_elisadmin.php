...
class block_elisadmin extends block_base {
    ...
    //Added by Faneeza Muskan
    public $release;
    public $section;
    public $lastcron;
    public $divcounter;
  
    //Ended by Faneeza Muskan
    ...
    /// Determine the users CM access level.
        ...
        //     // 1. Include the file that defines the generalized_filter_clustertree class.
        //     //    NOTE: You may need to adjust this path based on your Moodle file structure.
        //     require_once($CFG->dirroot . '/local/elisprogram/lib/filtering/clustertree.php');

        //     // 2. Create an instance of the class that contains the method we need.
        //     //    We can pass null values since we are not setting up a full form filter.
        //     $cluster_helper = new generalized_filter_clustertree(null, null, null, null, false, null);
        ...
        //     // 3. Call the method on our new helper object, NOT on $this.
        //     if ($all_clusters = $cluster_helper->cluster_dropdown_get_listing($context_result)) {
        //         // The result might be a data_collection object, so ensure it's a standard array.
        //         if (!is_array($all_clusters)) {
        //             $all_clusters = $all_clusters->to_array();
        //         }

        //         foreach ($all_clusters as $cluster) {
        //         // Determine the parent's name for the menu item structure.
        //         $parent_name = ($cluster->parent == 0) ? 'root' : 'userset_' . $cluster->parent;

        //         // CORRECTED: The action is now 'view' to generate the correct navigation link.
        //         $params = array('id'     => $cluster->id,
        //                         'action' => 'view');

        //         // Check if this cluster has any children to determine if it's a "leaf" node.
        //         $cluster_count = cluster_count_records('', '', array('parent' => $cluster->id));
        //         $isLeaf = empty($cluster_count);

        //         $cm_entity_pages[] = test_cluster_tree_get_menu_item(
        //                                  'cluster',
        //                                  $cluster,
        //                                  $parent_name, // This tells the tree where to place this node
   ...
   if (!isset(elis::$config->local_elisprogram->display_clusters_at_top_level) || !empty(elis::$config->local_elisprogram->display_clusters_at_top_level)) {
    ...
    $view_contexts = usersetpage::get_contexts('local/elisprogram:userset_view');
    $top_level_filter = array('contexts' => $view_contexts, 'parent' => 0);
    $num_records = cluster_count_records('', '', $top_level_filter);

    if ($clusters = cluster_get_listing('priority, name', 'ASC', 0, $num_block_icons, '', '', $top_level_filter)) {
       foreach($clusters as $cluster) {
            $params = array('id' => $cluster->id, 'action' => 'view', 's' => 'clst', 'currentitypath' => 'userset-' . $cluster->id);
            $sub_cluster_filter = array('contexts' => $view_contexts);
            $sub_cluster_count = cluster_count_records('', '', array('parent' => $cluster->id), $sub_cluster_filter);
            $program_contexts = curriculumpage::get_contexts('local/elisprogram:program_view');
            $program_count = clustercurriculum::count_curricula($cluster->id, array('contexts' => $program_contexts));
            $isLeaf = empty($sub_cluster_count) && empty($program_count);
            
            $link_params_for_row = $params;
            $instance_to_use = $cluster;

            if (!$isLeaf) {
                $link_params_for_row = array();
                $instance_to_use = clone $cluster;
                $url = new moodle_url('/local/elisprogram/index.php', $params);
                $attributes = [
                    'href'    => $url->out(false),
                    'onclick' => 'event.stopPropagation();'
                ];
                
                $instance_to_use->name = html_writer::tag('a', $cluster->name, $attributes);
            }

            $cm_entity_pages[] = block_elisadmin_get_menu_item('userset', $instance_to_use, 'root', $manageclusters_css_class, $cluster->id, 0, $link_params_for_row, $isLeaf);

            if (!$isLeaf && !empty($sub_cluster_count)) {
                $child_cluster_filter = array('contexts' => $view_contexts, 'parent' => $cluster->id);
                if ($child_clusters = cluster_get_listing('priority, name', 'ASC', 0, 0, '', '', $child_cluster_filter)) {
                    foreach ($child_clusters as $child_cluster) {
                        $c_params = array('id' => $child_cluster->id, 'action' => 'view', 's' => 'clst', 'currentitypath' => 'userset-' . $child_cluster->id);
                        $c_parent_name = 'userset_' . $cluster->id;
                        $grandchild_count = cluster_count_records('', '', array('parent' => $child_cluster->id), $sub_cluster_filter);
                        $c_isLeaf = empty($grandchild_count);

                        $cm_entity_pages[] = block_elisadmin_get_menu_item(
                            'userset', $child_cluster, $c_parent_name, $cluster_css_class, $child_cluster->id, 0, $c_params, $c_isLeaf
                        );
                    }
                }
            }
...
}  // Updated by Faneeza Muskan [02/10/2025]
...
if (!empty(elis::$config->local_elisprogram->display_curricula_at_top_level)) {
    // ======================================================================
    // ===== RECURSIVE HELPER FUNCTION START                            =====
    // ======================================================================
    // This new function is the core of the solution. It recursively fetches
    // and displays child user sets (clusters).

    if (!function_exists('block_elisadmin_add_child_clusters_recursively')) {
        function block_elisadmin_add_child_clusters_recursively(
            $parent_cluster_id,
            $parent_menu_name,
            &$cm_entity_pages, // Passed by reference to modify the main array
            $cluster_css_class,
            $filter
        ) {
            // Fetch all direct children of the given parent cluster.
            // Note: Depending on your ELIS version, you might need a different function here.
            // We assume a function get_records for the 'local_elisprogram_uset' table exists.
            global $DB;
            $child_clusters = $DB->get_records('local_elisprogram_uset', ['parent' => $parent_cluster_id], 'name ASC');

            if (empty($child_clusters)) {
                return; // This parent has no children, so we stop here.
            }

            foreach ($child_clusters as $child) {
                // Check if this child has its own children to determine if it's a leaf.
                $sub_cluster_count = $DB->count_records('local_elisprogram_uset', ['parent' => $child->id]);
                $is_leaf = empty($sub_cluster_count);

                // Prepare parameters for the menu item link.
                $params = ['id' => $child->id, 'action' => 'view'];

                // Add this child user set to the navigation menu.
                $cm_entity_pages[] = block_elisadmin_get_menu_item(
                    'userset', $child, $parent_menu_name, $cluster_css_class, $child->id, 0, $params, $is_leaf
                );

                // If this child is not a leaf, recurse deeper.
                if (!$is_leaf) {
                    $new_parent_menu_name = 'userset_' . $child->id;
                    block_elisadmin_add_child_clusters_recursively(
                        $child->id, $new_parent_menu_name, $cm_entity_pages, $cluster_css_class, $filter
                    );
                }
            }
        }
    ...
    foreach ($curricula as $curriculum) {
        ...
        if ($thismenuitem->page->page_instance->can_do()) {
            ...
            if ($validprogramcount <= $num_block_icons) {
                ...
                // If the program is not a leaf, fetch and add its child clusters (user sets).
                if (!$isLeaf && !empty($cluster_count)) {
                    if ($child_clusters = clustercurriculum::get_clusters($curriculum->id, 0, $cluster_filter)) {
                        foreach ($child_clusters as $cluster) {
                            // The object from get_clusters has the user set ID in 'clusterid'.
                            $top_level_cluster_id = $cluster->clusterid;
                            
                            // We need the full cluster record to get its name and check if it's a leaf.
                            $top_level_cluster_record = $DB->get_record('local_elisprogram_uset', ['id' => $top_level_cluster_id]);
                            if (!$top_level_cluster_record) {
                                continue; // Skip if the cluster record doesn't exist.
                            }

                            $c_params = array('id' => $top_level_cluster_id, 'action' => 'view');
                            $c_parent_name = 'curriculum_' . $curriculum->id;

                            // Check if this top-level cluster has children of its own.
                            $sub_cluster_count = $DB->count_records('local_elisprogram_uset', ['parent' => $top_level_cluster_id]);
                            $c_isLeaf = empty($sub_cluster_count);

                            // Add the top-level cluster to the menu.
                            $cm_entity_pages[] = block_elisadmin_get_menu_item(
                                'userset', $top_level_cluster_record, $c_parent_name, $cluster_css_class, $top_level_cluster_id, 0, $c_params, $c_isLeaf
                            );

                            // ***MODIFICATION***
                            // If this cluster is not a leaf, call our new recursive function to fetch all its descendants.
                            if (!$c_isLeaf) {
                                $new_parent_menu_name = 'userset_' . $top_level_cluster_id;
                                block_elisadmin_add_child_clusters_recursively(
                                    $top_level_cluster_id,
                                    $new_parent_menu_name,
                                    $cm_entity_pages,
                                    $cluster_css_class,
                                    $cluster_filter
                                );
    ...
    $curricula->close(); // Close the recordset iterator.

    if ($num_block_icons < $validprogramcount) {
        $cm_entity_pages[] = block_elisadmin_get_menu_summary_item('curriculum', $curriculum_css_class, $validprogramcount - $num_block_icons);
    }
} //Updated by Faneeza Muskan to display Curricula at the top level of the menu [16/07/20205]
